document.addEventListener('DOMContentLoaded', function() {
    ...
    initialAssets.forEach(asset => { addAssetToTable(asset.category, asset.type, asset.quantity, asset.availability); });
});
function addAsset() {
...
function searchAssets() {
    ...
    const tableRows = document.getElementById('assetTable').getElementsByTagName('tbody')[0].getElementsByTagName('tr');

    for (let i = 0; i < tableRows.length; i++) {
        const categoryCell = tableRows[i].getElementsByTagName('td')[0];
        if (categoryCell) {
            const categoryText = categoryCell.textContent || categoryCell.innerText;
            tableRows[i].style.display = categoryText.toLowerCase().indexOf(searchInput) > -1 ? '' : 'none';
        }
    }
}